      ...
      const layer = L.geoJSON(grid, {
        ...
        style: function(f) {
          ...
          return {
            ...
            color: isSel ? '#111' : isTop ? '#111' : 'rgba(255,255,255,.6)',
            weight: isSel ? 3 : isTop ? 2 : 0.5
          };
        },
  ...
  return (
    <main className="site-shell">
        ...
        <aside className="inspect">
          ...
          {[
            ...
            <div className="meter" key={key}>
              <span>{label}<b>{Number(p?.[key] ?? 0).toFixed(0)}</b></span>
      ...
      <section className="method" id="method">
        ...
        <div className="chain">
          {['Official boundaries', 'Aligned 500 m grid', 'Surface + nature + population', 'Four normative scenarios', 'Independent Landsat validation', 'Spatial robustness tests', 'Human interpretation'].map((x, i) => (
            <div key={x}>
              <span>{String(i + 1).padStart(2, '0')}</span>
              <b>{x}</b>
            </div>
